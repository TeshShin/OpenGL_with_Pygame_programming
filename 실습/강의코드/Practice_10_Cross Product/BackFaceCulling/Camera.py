...
class Camera:
    def __init__(self, fovy, aspect, near, far):
        ...
        self.mouse_invert = -1
        self.forward = pygame.Vector3(self.get_VM()[0, 2], self.get_VM()[1, 2], self.get_VM()[2, 2])
    ...
    def update(self):
        ...
        if key[pygame.K_w]:
            self.transform.update_position(self.transform.get_position() + pygame.Vector3(0, 0, self.pan_speed), True)
        if key[pygame.K_s]:
            self.transform.update_position(self.transform.get_position() + pygame.Vector3(0, 0, -self.pan_speed), True)
        if key[pygame.K_a]:
            self.transform.update_position(self.transform.get_position() + pygame.Vector3(self.pan_speed, 0, 0), True)
        if key[pygame.K_d]:
            self.transform.update_position(self.transform.get_position() + pygame.Vector3(-self.pan_speed, 0, 0), True)
        if key[pygame.K_q]:
    ...
    def rotate_with_mouse(self, yaw, pitch):
        ...
        angle = self.forward.angle_to(up)
